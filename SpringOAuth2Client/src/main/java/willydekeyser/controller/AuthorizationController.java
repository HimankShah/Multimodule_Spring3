...
public class AuthorizationController {
	...
	public String user(Model model,
			@RegisteredOAuth2AuthorizedClient("springoauthclient") OAuth2AuthorizedClient authorizedClient) {

		String messages = this.webClient.get().uri(this.messagesBaseUri + "/user")
	...
	public String admin(Model model,
		...
		String messages = this.webClient.get().uri(this.messagesBaseUri + "/admin")
				.attributes(oauth2AuthorizedClient(authorizedClient)).retrieve().bodyToMono(String.class).block();
		model.addAttribute("messages", messages);
		model.addAttribute("login", true);
		return "index";
